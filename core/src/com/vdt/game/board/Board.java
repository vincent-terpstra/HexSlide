...
public class Board {
    ...
    public int score = 0;
    public int max = 5;
    private final void set(int x, int y, int idx){
    ...
    void addRandom(final int total, EffectManager effects){
        int open = 0;
        for(int b : array){
            if (b == 0)
                open++;
    ...
    static boolean inHex(float x, float y){
    ...
    public final boolean isEqual(Board board){
        int[] otherBoard = board.array;
        for(int i = 0; i < array.length; i++){
            if(array[i] != otherBoard[i])
                return false;
        }
        return true;
    }
